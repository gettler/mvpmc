...
LIB="libinput.so libgw.so libav.so libcmyth.so libdemux.so librefmem.so libosd.so libfreetype.so.6.3.10 libfreetype.so.6 libfreetype.so libmicrohttpd.so libmicrohttpd.so.4.0.2 libmicrohttpd.so.4 libcurl.so libcurl.so.4 libcurl.so.4.0.1"
