...
LIB="libinput.so libgw.so libav.so librefmem.so libosd.so libfreetype.so.6.3.10 libfreetype.so.6 libfreetype.so libmicrohttpd.so libmicrohttpd.so.4.0.2 libmicrohttpd.so.4 libcurl.so libcurl.so.4 libcurl.so.4.0.1"
PLUGINS="libmvpmc_osd.plugin libmvpmc_html.plugin libmvpmc_http.plugin libmvpmc_av.plugin libmvpmc_screensaver.plugin"
